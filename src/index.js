...
class Scheduler extends Component {
    ...
    render() {
        ...
        const isReversed = width < config.reverseBreakpoint;
        ...
        if (showAgenda) {
            ...
            let resourceTableWidth = isReversed ? schedulerData.getSchedulerWidth() : schedulerData.getResourceTableWidth();
            let schedulerContainerWidth = isReversed ? schedulerData.getSchedulerWidth() : (width - resourceTableWidth + 1);
            let schedulerWidth = isReversed ? schedulerData.getReversedContentTableWidth() : schedulerData.getContentTableWidth() - 1; // is reversed ? width- timetableWidth
            ...
            let resourceContentStyle = { overflowX: "auto", overflowY: "auto", width: isReversed ? null : (resourceTableWidth + resourceScrollbarWidth - 2), margin: `0px -${contentScrollbarWidth}px 0px 0px` };
            ...
            let resourceName = schedulerData.isEventPerspective ? config.taskName : isReversed ? config.resourceNameReversed : config.resourceName;
            tbodyContent = (
                <tr>
                    <td style={{ width: resourceTableWidth, verticalAlign: 'top' }}>
                        <div className="resource-view">
                            <div className="resource-view__name" style={{ overflow: "hidden", borderBottom: "1px solid #e9e9e9", height: isReversed ? '' : config.tableHeaderHeight }}>
                                <div style={{ overflowX: "scroll", overflowY: "hidden", margin: `0px 0px -${contentScrollbarHeight}px` }}>
                                    <table className="resource-table">
                                        <thead>
                                            <tr style={{ height: isReversed ? '44px' : config.tableHeaderHeight }}>
                            ...
                            <div className="resource-view__list" style={resourceContentStyle} ref={this.schedulerResourceRef} onMouseOver={this.onSchedulerResourceMouseOver} onMouseOut={this.onSchedulerResourceMouseOut} onScroll={() => this.onSchedulerResourceScroll(isReversed)}>
                                <ResourceView
                                    isReversed={isReversed}
                                    {...this.props}
                    ...
                    <td>
                        <div className="scheduler-view" style={{ width: schedulerContainerWidth, verticalAlign: 'top' }}>
                            <div className="header-wrapper" style={{ 
                                ...
                                <div style={{ overflowX: isReversed ? null : "scroll", overflowY: "hidden", margin: `0px 0px -${contentScrollbarHeight}px` }} ref={this.schedulerHeadRef} onMouseOver={this.onSchedulerHeadMouseOver} onMouseOut={this.onSchedulerHeadMouseOut} onScroll={this.onSchedulerHeadScroll}>
                                    <div style={{ paddingRight: `${contentScrollbarWidth}px`, width: isReversed ? null : (schedulerWidth + contentScrollbarWidth) }}>
                                        <table className="scheduler-bg-table scheduler-view__header">
                                            <HeaderView isReversed={isReversed} {...this.props} />
                                        </table>
                                    </div>
                                </div>
                            </div>
                            <div onScroll={this.handleScroll} style={schedulerContentStyle} ref={this.schedulerContentRef} onMouseOver={this.onSchedulerContentMouseOver} onMouseOut={this.onSchedulerContentMouseOut} onScroll={() => this.onSchedulerContentScroll(isReversed)} >
                                <div style={{ width: schedulerWidth, height: contentHeight }}>
                                    ...
                                    <div className="scheduler-bg" style={{ overflow: 'hidden' }}>
        ...
        return (
            <table id="RBS-Scheduler-root" className={`scheduler ${isReversed ? 'scheduler-reverse' : ''}`} style={{ width: `${width}px` }}>
                <thead>
                    <tr>
                        <td colSpan="2">
                            {schedulerHeader}
    ...
    onSchedulerResourceScroll = (isReversed) => {
        ...
        if (isReversed) {
            ...
            if (!this.state.scrollingResource) {
                this.setState({
                    scrollingContent: true,
                }, () => {
                    this.schedulerContent.scrollLeft = this.schedulerResource.scrollLeft; 
                })
            } else {
                this.setState({
                    scrollingResource: false,
    ...
    onSchedulerContentScroll = (isReversed) => {
        ...
        if (isReversed) {
            // Prevent onscroll loop
            if (!this.state.scrollingContent) {
                this.setState({
                    scrollingResource: true,
                }, () => {
                    this.schedulerResource.scrollLeft = this.schedulerContent.scrollLeft; 
                })
            } else {
                this.setState({
                    scrollingContent: false,
