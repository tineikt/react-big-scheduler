...
var SchedulerData = function () {
    ...
    _createClass(SchedulerData, [{
        ...
        value: function getSchedulerWidth() {
            var documentWidth = this.documentWidth > 0 ? this.documentWidth : document.documentElement.clientWidth;
            var isReversed = documentWidth < this.config.reverseBreakpoint;
            var besidesWidth = isReversed ? 0 : this.config.besidesWidth;
            var baseWidth = documentWidth - besidesWidth > 0 ? documentWidth - besidesWidth : 0;
        ...
        key: 'getReversedContentCellWidth',
        value: function getReversedContentCellWidth(rowHeight) {
            return this.config.columnReversedWidth * Math.floor(rowHeight / this.config.eventItemLineHeight);
        }
    }, {
        key: 'getContentTableWidth',
        value: function getContentTableWidth() {
            var width = this.headers.length * this.getContentCellWidth();
            if (this.cellUnit === _index.CellUnits.Hour) width = width / this.getMinuteStepsInHour();
            return width;
        }
    }, {
        ...
        value: function getSchedulerContentDesiredHeight() {
            var height = 0;
            this.renderData.forEach(function (item) {
                if (item.render) height += item.rowHeight + 10;
            });
            return height;
        }
    }, {
        key: 'getReversedContentTableWidth',
        value: function getReversedContentTableWidth() {
            var _this = this;

            var width = 0;
            this.renderData.forEach(function (item) {
                if (item.render) width += _this.getReversedContentCellWidth(item.rowHeight);
            });
            return width;
        }
    }, {
        ...
        value: function _attachEvent(event) {
            ...
            this.events.forEach(function (item, index) {
                var start = _this2.localeMoment(item.start);
        ...
        value: function _handleRecurringEvents() {
            ...
            recurringEvents.forEach(function (item) {
                _this3._detachEvent(item);
            });

            recurringEvents.forEach(function (item) {
                var windowStart = _this3.localeMoment(_this3.startDate),
                    windowEnd = _this3.localeMoment(_this3.endDate).add(1, 'days'),
                    oldStart = _this3.localeMoment(item.start),
                    oldEnd = _this3.localeMoment(item.end),
                    rule = (0, _rrule.rrulestr)(item.rrule),
                    oldDtstart = undefined;
                if (!!rule.origOptions.dtstart) {
                    oldDtstart = _this3.localeMoment(rule.origOptions.dtstart);
                }
                //rule.origOptions.dtstart = oldStart.toDate();
                if (!rule.origOptions.until || windowEnd < _this3.localeMoment(rule.origOptions.until)) {
                ...
                if (item.exdates || item.exrule) {
                    ...
                    if (item.exdates) {
                        item.exdates.forEach(function (exdate) {
                            rruleSet.exdate(_this3.localeMoment(exdate).toDate());
                        });
                    }
                    rule = rruleSet;
                }
                ...
                var newEvents = all.map(function (time, index) {
                    return _extends({}, item, {
                        ...
                        id: item.id + '-' + index,
                        start: rule.origOptions.tzid ? _this3.localeMoment.utc(time).utcOffset(_this3.localeMoment().utcOffset(), true).format(_index.DATETIME_FORMAT) : _this3.localeMoment(time).format(_index.DATETIME_FORMAT),
                        end: rule.origOptions.tzid ? _this3.localeMoment.utc(time).utcOffset(_this3.localeMoment().utcOffset(), true).add(oldEnd.diff(oldStart), 'ms').format(_index.DATETIME_FORMAT) : _this3.localeMoment(time).add(oldEnd.diff(oldStart), 'ms').format(_index.DATETIME_FORMAT)
                    });
                });
                newEvents.forEach(function (newEvent) {
                    var eventStart = _this3.localeMoment(newEvent.start),
                        eventEnd = _this3.localeMoment(newEvent.end);
                    if (_this3.isEventInTimeWindow(eventStart, eventEnd, windowStart, windowEnd) && (!oldDtstart || eventStart >= oldDtstart)) {
                        _this3._attachEvent(newEvent);
                    }
                });
            });
        }
        ...
        value: function _createRenderData() {
            ...
            this.events.forEach(function (item) {
                ...
                if (resourceEventsList.length > 0) {
                    ...
                    var eventStart = _this6.localeMoment(item.start),
                        eventEnd = _this6.localeMoment(item.end);
                    var pos = -1;
                    // console.log("re l", resourceEvents.headerItems.length);
                    resourceEvents.headerItems.forEach(function (header, index) {
                        var headerStart = _this6.localeMoment(header.start),
                            headerEnd = _this6.localeMoment(header.end);
                        if (headerEnd > eventStart && headerStart < eventEnd) {
                            ...
                            if (render === false) {
                                ...
                                var previousHeaderStart = _this6.localeMoment(previousHeader.start),
                                    previousHeaderEnd = _this6.localeMoment(previousHeader.end);
